...
def cmp_to_symbol(val, other_val):
    '''
    Return the symbol representing the relationship between two values
    '''
    if val > other_val:
        return 1
    elif val < other_val:
...
class GeoCoord(metaclass=abc.ABCMeta):
    ...
    def _calc_degreeminutes(decimal_degree):
        ...
        '''
        sign = cmp_to_symbol(decimal_degree, 0) # Store whether the coordinate is negative or positive
